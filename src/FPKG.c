		...
		while ((readval = getc(file)) != EOF) {
			...
			if (readval == '\n') {
				...
				*/
				...
				extract_values(test_data,value,785);
		...
		if (file) {
			...
			while ((readval = getc(file)) != EOF) {
						...
						if (readval == '\n') {
							...
							extract_values(test_data2,value2,785);

							if (value2[0] == 1 || value2[0] == 0 ){

								output = calculate_output(perceptron,value2,sizeof(perceptron));
								int error = evaluate(output, value2[0]);

								if (error == 1){ correct++;} else { wrong++;}
								total++;
